...
def returnRadios():
    ...
    if request.method == "GET":
        ...
        if request.form["operations"] == "Validate Configuration":
            return "Call Validate config Endpoints"
...
def addConfig():
    ...
    #print("Print Login session = {}".format(session["logged_in"]))
    if request.method == "GET" and session["logged_in"] == False:
    ...
    elif request.method == "POST" and session["logged_in"] == False:
        return redirect (url_for("authenticateUsers"))
    ...
    if request.method == "POST":
        ...
        if addingConfiglet.status_code == 200:
            return "Configlet Added Successfully"
        else: 
